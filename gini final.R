...
ggplot(df_plot, aes(x = ano, y = proporcao, fill = tipo_gasto)) +
  ...
  facet_wrap(~ Capitais, scales = "free_x", ncol = 5) + # Ajusta o número de colunas
  labs(title = "Participação no gasto por fonte por Capital (2002 - 2022)",
       x = "Ano",
       y = "Proporção (%)",
       fill = "Tipo de Fonte") +
  ...
  theme(legend.title = element_blank(),
        ...
        panel.spacing = unit(0.5, "lines")) # Aumenta o espaço entre os painéis
...
#exportando excel
wb <- createWorkbook()
addWorksheet(wb, "Estatísticas por Região")

# Exportar a tabela para Excel
writeDataTable(
  wb, 
  sheet = "Estatísticas por Região", 
...
ggplot(estatisticas_transferencias, aes(x = ano, y = transferencia_per_capita, group = Capitais, color = Capitais)) +
  geom_line() +
  facet_wrap(~ Capitais, scales = "free_y", ncol = 5, labeller = label_wrap_gen()) +  # Corrigido parêntese
  labs(
    title = "Receitas de transferências per Capita por Capital (2002 - 2022)", 
    x = "Ano",
    y = "Transferência per Capita (R$)"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),  # Melhorar legibilidade dos rótulos no eixo x
    strip.text = element_text(size = 10),  # Ajustar tamanho dos títulos dos painéis
    panel.spacing = unit(0.8, "lines")  # Espaçamento entre os painéis
  ) +
...
latex_tabela_gini_por_regiao_capitais <- kable(gini_por_regiao_capitais, format = "latex", caption = "Gini por Região para as Capitais") %>%
  kable_styling(full_width = FALSE)
...
latex_tabela_gini_por_municipio <- kable(gini_por_municipio, format = "latex", caption = "Gini por Município para as Capitais") %>%
  kable_styling(full_width = FALSE)
...
ggplot(dados_capitais, aes(x = regiao, y = despesa_asps_2022)) +
  ...
  labs(
    title = "Boxplot do Gasto Total em ASPS por Região",
    x = "Região", 
    y = "Gasto Total em ASPS"
  ) +
